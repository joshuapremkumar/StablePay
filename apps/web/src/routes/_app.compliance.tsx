...
function Compliance() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["demo", "compliance"],
    queryFn: demoApi.getCompliance,
  });

  const screening = useMutation({
    mutationFn: () => demoApi.screenEntity("Shenzhen Components Ltd"),
    onSuccess: async (result: any) => {
      await queryClient.invalidateQueries({ queryKey: ["demo"] });
      toast.success(
        result.overall_status === "flagged"
          ? "Simulated a sanctions flag and audit event."
          : "Simulated a clean compliance screen.",
      );
    },
  });

  if (isLoading) {
    return <div className="p-6 md:p-8">Loading compliance simulation...</div>;
  }

  if (error || !data) {
    return <div className="p-6 md:p-8">Unable to load the compliance simulation.</div>;
  }
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto">
      <PageHeader
        ...
        actions={
          <Button variant="outline" size="sm" onClick={() => screening.mutate()}>
            <Download className="h-4 w-4 mr-1.5" /> Run sanctions screen
          </Button>
        }
      />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KpiCard label="KYB status" value={data.summary.kybStatus} icon={ShieldCheck} hint="All entities" />
        <KpiCard label="AML flags (30d)" value={`${data.summary.amlFlags30d}`} icon={AlertTriangle} hint="Live demo count" />
        <KpiCard label="Audit events" value={`${data.summary.auditEvents}`} delta={0.083} icon={FileCheck} />
        <KpiCard label="Active sessions" value={`${data.summary.activeSessions}`} icon={Lock} hint="All MFA-enforced" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <SectionCard
          title="KYB / KYC status"
          description="Per legal entity"
          className="lg:col-span-2"
        >
          <div className="space-y-3">
            {data.kycEntities.map((e) => (
              <div
                key={e.entity}
                className="flex items-center gap-3 rounded-lg border border-border bg-surface-elevated p-3"
              >
                <div className="h-9 w-9 rounded-md bg-surface grid place-items-center">
                  <ShieldCheck
                    className={`h-4 w-4 ${e.status === "verified" ? "text-success" : "text-warning"}`}
                  />
                </div>
                <div className="flex-1">
                  <div className="text-sm font-medium">{e.entity}</div>
                  <div className="text-[11px] text-muted-foreground">
                    {e.type} / expires {e.expires}
                  </div>
                </div>
              ...
              <div key={r.role} className="rounded-lg border border-border p-3">
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {r.permissions.map((p) => (
                    <span
                      key={p}
                      className="text-[10px] px-1.5 py-0.5 rounded border border-border bg-surface-elevated text-muted-foreground"
                    >
                      {p}
                    </span>
                  ))}
              ...
              <tbody>
                {data.auditLog.map((e) => (
                  <tr key={e.id} className="border-b border-border/60 hover:bg-surface-elevated">
                    ...
                    <td className="px-5 py-3 text-[11px] text-muted-foreground tabular">
                      {e.timestamp} / {e.ip}
                    </td>
                  </tr>
                ))}
      ...
      <SectionCard title="AML flags" description="Escalated entities and review notes">
        <div className="space-y-3">
          {data.amlFlags.map((flag) => (
            <div key={flag.id} className="rounded-lg border border-border bg-surface-elevated p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-sm font-medium">{flag.entity}</div>
                  <div className="text-[11px] text-muted-foreground">
                    {flag.flag_type} / {flag.created_at}
                  </div>
                </div>
                <StatusPill status={flag.status} />
              </div>
              <div className="text-sm text-muted-foreground mt-2">{flag.description}</div>
            </div>
          ))}
        </div>
      </SectionCard>
